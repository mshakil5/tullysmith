...
<div class="container-fluid">
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header"><h5 class="card-title mb-0"><i class="ri-list-check me-2 text-primary"></i>Time Entries</h5></div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0 report-table">
                    ...
                    <tbody id="logsTable"><tr><td colspan="7" class="text-center text-muted py-4">Loading...</td></tr></tbody>
                    <tfoot>
                        <tr>
                            <td colspan="6" class="text-end fw-bold">Total Hours</td>
                            <td class="text-end fw-bold" id="footTotalHours">—</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
<script>
$(function() {
    var mode = 'weekly', offset = 0, chartInst = null, chartType = 'bar';
    ...
    function load() {
        ...
        $('#logsTable').html('<tr><td colspan="7" class="text-center py-3"><span class="spinner-border spinner-border-sm"></span></td></tr>');
        $('#footTotalHours').text('—');
        ...
        $.get('{{ route("reports.time.data") }}', params(), function(r) {
            ...
            $('#footTotalHours').text(r.total_hours + 'h');

            // Chart
            var hasData = r.graph_hours && r.graph_hours.some(function(h){ return h > 0; });
            if (!hasData || !r.graph_labels.length) {
                $('#reportChart').hide(); $('#chartEmpty').removeClass('d-none');
            } else {
                $('#reportChart').show(); $('#chartEmpty').addClass('d-none');
                renderChart(r.graph_labels, r.graph_hours, r.graph_sessions);
            }

            // Chronological logs table
            if (!r.logs || !r.logs.length) {
                $('#logsTable').html('<tr><td colspan="7" class="text-center text-muted py-4">No data for this period</td></tr>');
                return;
            }
            ...
            r.logs.forEach(function(l, i) {
                ...
                html += '<tr class="' + rowClass + '">' +
                    ...
                    '<td>' + l.worker + '</td>' +
                    '<td><span class="fw-semibold">' + l.job + '</span><div class="text-muted small">' + l.job_id + '</div></td>' +
                    '<td class="text-center small">' + l.clock_in + '</td>' +
                    '<td class="text-center small">' + (l.clock_out === 'Active' ? '<span class="badge bg-warning text-dark">Active</span>' : l.clock_out) + '</td>' +
    ...
    function renderChart(labels, hours, sessions) {
        ...
        chartInst = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [
                    { label: 'Hours', data: hours, backgroundColor: 'rgba(99,102,241,.7)', borderColor: 'rgba(99,102,241,1)', borderWidth:2, borderRadius: chartType==='bar'?4:0, tension:.4, yAxisID:'y' },
                    { label: 'Sessions', data: sessions, type: chartType==='bar'?'bar':'line', backgroundColor: 'rgba(34,197,94,.5)', borderColor: 'rgba(34,197,94,1)', borderWidth:2, borderRadius: chartType==='bar'?4:0, tension:.4, fill:false, yAxisID:'y1' }
                ]
            },
            options: {
                responsive:true,
                ...
                scales:{
                    ...
                    y1: { type:'linear', position:'right', title:{ display:true, text:'Sessions' }, beginAtZero:true, grid:{ drawOnChartArea:false } }
                }
            }
        });
    }
