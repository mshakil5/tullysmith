...
@section('content')
    <div class="container-fluid">
            ...
            <table width="100%" style="border-bottom:2px solid #1a2d52;padding-bottom:12px;margin-bottom:16px;">
                <tr>
                    ...
                    <td width="30%" style="text-align:right;vertical-align:top;">
                        ...
                        <div style="font-size:14px;font-weight:600;border-bottom:2px solid #dee2e6;display:inline-block;">
                            Date
                        </div>
                        <div style="font-size:14px;font-weight:600;color:#1a2d52;margin-top:2px;" id="p_period_label">—
                        </div>
                        ...
                        <div style="font-size:14px;font-weight:600;color:#1a2d52;margin-top:2px;" id="p_job_label">All Jobs
                        </div>
        ...
        <div class="alert alert-success border-success mb-4 d-flex align-items-center gap-3" id="totalWordsBanner"
    ...
    <script>
        $(function() {
            ...
            function load() {
                ...
                $('#statTotal,#statCount,#statWorkers,#statJobs').html(
                ...
                $.get('{{ route('reports.expense.data') }}', params(), function(r) {
                    ...
                    theadHtml +=
                        '<th>Title</th><th class="text-end">Amount</th><th class="no-print text-center">File</th></tr>';
                    $('#expensesHead').html(theadHtml);

                    // Dynamic tfoot
                    var tfootColspan = isJobFiltered ? 3 : 4;
                    var tfootHtml = '<tr><td colspan="' + tfootColspan +
                        '" class="text-end fw-bold">Total Amount</td>' +
                        '<td class="text-end fw-bold" id="footTotal">£' + r.total_amount + '</td>' +
                        '<td class="no-print"></td></tr>';
                    $('#expensesFoot').html(tfootHtml);

                    if (!r.expenses || !r.expenses.length) {
                        $('#expensesTable').html('<tr><td colspan="' + colspan +
                    ...
                    r.expenses.forEach(function(e, i) {
                        ...
                        if (!isJobFiltered) {
                            html += '<td><span class="fw-semibold">' + e.job +
                                '</span><div class="text-muted small">' + e.job_id + '</div></td>';
                        }
                        html += '<td>' + e.title + '</td>' +
                            '<td class="text-end fw-bold text-success">£' + e.amount + '</td>' +
                            '<td class="text-center no-print">' +
                            (e.file ? '<a href="' + e.file +
                                '" target="_blank" class="btn btn-xs btn-soft-secondary"><i class="ri-eye-line"></i></a>' :
                                '—') +
                            '</td>' +
                            '</tr>';
                        prevDate = e.date;
