    ...
    <div class="container-fluid">
            ...
            <table width="100%" style="border-bottom:2px solid #1a2d52; padding-bottom:8px; margin-bottom:16px;">
                <tr>
                    <td width="70%" style="vertical-align: bottom;">
                        ...
                        @endif
                        <div style="font-size:22px;font-weight:700;color:#1a2d52;letter-spacing:1px; line-height: 1.2;">CHECKLIST REPORT</div>
                    ...
                    <td width="30%" class="print-header-box" style="text-align:right; vertical-align: bottom;">
                        <div style="display: inline-block; text-align: left; padding-bottom: 4px;">
                            <div class="mb-1">
                                <span class="small fw-semibold" style="min-width: 85px; display: inline-block;">Date:</span>
                                <span class="small fw-semibold" id="p_period_label">—</span>
                            </div>

                            <div class="mb-1">
                                <span class="small fw-semibold" style="min-width: 85px; display: inline-block;">Worker:</span>
                                <span class="small fw-semibold" id="p_worker_label">All Workers</span>
                            </div>

                            <div class="mb-0">
                                <span class="small fw-semibold" style="min-width: 85px; display: inline-block;">Job/Project:</span>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header">
                <h5 class="card-title mb-0"><i class="ri-checkbox-multiple-line me-2 text-info"></i>Checklist Answers</h5>
            </div>
...
@section('script')
    <script>
        $(function() {
            ...
            function load() {
                $('#statAnswers,#statChecklists,#statWorkers,#statJobs').html(
                    '<span class="spinner-border spinner-border-sm"></span>');
                $('#answersTable').html(
                    '<tr><td colspan="9" class="text-center py-3"><span class="spinner-border spinner-border-sm"></span></td></tr>'
                    );
                $('#answersHead').html('');
                ...
                $.get('{{ route('reports.checklist.data') }}', params(), function(r) {
                    ...
                    theadHtml +=
                        '<th>Checklist</th><th>Question</th><th>Answer</th><th class="text-center no-print">Photo</th></tr>';
                    $('#answersHead').html(theadHtml);

                    if (!r.answers || !r.answers.length) {
                        $('#answersTable').html('<tr><td colspan="' + totalCols +
                    ...
                    r.answers.forEach(function(a, i) {
                        ...
                        var showAtLabel = a.show_at === 'clock_in' ? 'Clock In' : (a.show_at ===
                            'clock_out' ? 'Clock Out' : 'Both');
                        ...
                        if (a.type === 'photo' && a.photo) {
                            ...
                            answerHtml = '<span class="badge bg-' + ansClass + '">' + a.answer +
                                '</span>';
                        } else {
                            answerHtml = a.answer || '—';
                        }
                        ...
                        if (!isJobFiltered) html += '<td><span class="fw-semibold">' + a.job +
                            '</span><div class="text-muted small">' + a.job_id + '</div></td>';
                        html += '<td><span class="fw-semibold">' + a.checklist +
                            '</span><div><span class="' + showAtClass + '">' + showAtLabel +
                            '</span></div></td>' +
                            '<td class="small">' + a.question + '</td>' +
                            '<td>' + answerHtml + '</td>' +
                            '<td class="text-center no-print">' +
                            (a.photo ? '<a href="' + a.photo +
                                '" target="_blank" class="btn btn-xs btn-soft-info"><i class="ri-image-line"></i></a>' :
                                '—') +
                            '</td>' +
                            '</tr>';
                        prevDate = a.date;
