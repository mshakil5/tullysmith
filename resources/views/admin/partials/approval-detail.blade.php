@php
    if ($type === 'timelog') {
        ...
        $jobTitle = $item->assignment
            ? $item->assignment->formatted_date .
                ($item->assignment->start_time || $item->assignment->end_time
                    ? ' · ' .
                        ($item->assignment->start_time
                            ? \Carbon\Carbon::parse($item->assignment->start_time)->format('h:i A')
                            : '--') .
                        ' — ' .
                        ($item->assignment->end_time
                            ? \Carbon\Carbon::parse($item->assignment->end_time)->format('h:i A')
                            : '--')
                    : '')
            : $item->clock_in_at->format('d F Y');
        ...
        $status =
            $item->status === 'completed' ? 'pending' : ($item->status === 'archived' ? 'approved' : $item->status);
...
<div class="p-1">
            ...
            <div class="col-6">
                ...
                @else
                    <div class="bg-light rounded-3 d-flex align-items-center justify-content-center text-muted"
                        style="height:100px;">No Image</div>
                @endif
            </div>
            <div class="col-6">
    ...
    @elseif ($type === 'servicejob')
        ...
        <div class="d-flex justify-content-between align-items-center border rounded p-3 mb-3">
            ...
            <span
                class="badge bg-{{ $item->priority === 'low' ? 'success' : ($item->priority === 'medium' ? 'warning' : 'danger') }}">{{ ucfirst($item->priority) }}</span>
        </div>
                    ...
                    @if ($answer)
                        <div class="mt-2">
                            @if ($answer->photo_path)
                                <img src="{{ asset($answer->photo_path) }}" class="img-fluid rounded mb-2"
    ...
    @else
        <div class="alert alert-{{ $status === 'approved' ? 'success' : 'danger' }} mt-3 mb-0">
            This item has been
            <strong>{{ $type === 'servicejob' ? ($status === 'approved' ? 'archived' : 'Sent for Redo') : ucfirst($status) }}</strong>.
