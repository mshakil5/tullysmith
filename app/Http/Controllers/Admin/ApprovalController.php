        ...
        if ($request->ajax()) {
            ...
            $notes = Note::with(['job.client', 'user'])
                ...
                ->map(fn($n) => [
                    ...
                    'type'       => 'note',
                    'title'      => $n->note,
                    'job'        => $n->job->job_title ?? 'N/A',
                    'client'     => $n->job->client->name ?? 'N/A',
                    'created_by' => $n->user->name ?? 'N/A',
            ...
            $documents = Document::with(['job.client', 'user'])
                ...
                ->map(fn($d) => [
                    ...
                    'job'        => $d->job->job_title ?? 'N/A',
                    'client'     => $d->job->client->name ?? 'N/A',
                    'created_by' => $d->user->name ?? 'N/A',
            ...
            $checklists = ServiceJobChecklist::with(['serviceJob.client', 'checklist', 'assignedBy'])
                ...
                ->map(fn($c) => [
                    'id'         => $c->id,
                    'type'       => 'checklist',
                    'title'      => $c->checklist->title ?? 'N/A',
                    'job'        => $c->serviceJob->job_title ?? 'N/A',
                    'client'     => $c->serviceJob->client->name ?? 'N/A',
                    'created_by' => $c->assignedBy->name ?? 'N/A',
